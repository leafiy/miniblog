<template>
  <div v-if="about">
    ...
    <div class="hr"></div>
    <UIButton type="primary" @click="$router.push('/admin/create/article/'+$route.params.panel)">新建Research</UIButton>
    <div class="hr"></div>
    <div v-if="researchList" v-for="item of researchList">
  ...
  methods: {
    ...
    load() {
      ...
      api.getList(param).then(response => {
        this.researchList = response.data.articleList
      }).catch(error => {
        console.log(error)
      })
    }
